...
def submit_answer():
    ...
    selected_answer = data.get("selected")
    correct_answer = data.get("correct")
    options = data.get("options")

    if not all([username, question, selected_answer, correct_answer, options]):
    ...
    return jsonify({"message": "Answer stored successfully"}), 200
...
stripe.api_key = "sk_test_51RT0w6PqLkT24mGSQWPMAxYcD1fm1wIS9dIZCc7N0qqKbBWif1xyNl3tgCLTurDHHDhFMzXFneebXkrIH9iDoBfX00M6SnfLdz"

@app.route('/create-payment-intent', methods=['POST'])
def create_payment_intent():
    try:
        data = request.json
        amount = data.get('amount')  # Amount in cents (e.g., 500 for $5.00 AUD)
